...
# added for Non adaptive

class DiagnosticItemDTO(BaseModel):
    """DTO representing a single diagnostic item (speaking or writing)."""
    item_id: str
...
class DiagnosticSessionDTO(BaseModel):
    """DTO representing a diagnostic session and all its items."""
    session_id: str
    assessment_id: str
    items: List[DiagnosticItemDTO]

    @classmethod
    def from_domain(cls, session, items):
        """
        Build DTO from domain session + list of domain AssessmentItem value objects.
        """
        return cls(
            session_id=str(session.id),
            assessment_id=str(session.assigned_assessment_id),
            items=[
                DiagnosticItemDTO(
                    item_id=str(item.id),
...
class DiagnosticResultDTO(BaseModel):
    """DTO representing the result of a diagnostic assessment (speaking/writing)."""
    session_id: str
